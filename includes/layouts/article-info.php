...
<div class="panel panel-default">
	<div class="panel-heading">
		...
		<?php if ($session->is_admin_logged_in()): ?>
			<a class="btn btn-primary"
			   href="edit_article.php?subject=<?php echo urlencode($current_subject->id); ?>&article=<?php echo urlencode($current_article->id); ?>"
		...
		<h5>
			<?php if (isset($current_article->author_id)): ?>
				...
				if ( ! empty($_author->photo)): ?>
					<img class="author-photo img-circle pull-left" alt="<?php echo $_author->full_name(); ?>"
					     src="data:image/jpeg;base64,<?php echo base64_encode($_author->photo); ?>"/>
				<?php endif; ?>
			<?php endif; ?>
		</h5>
		<h5>
		...
		<article id="comments">
			<?php if ($session->is_logged_in()): ?>
				...
				<form class="form-horizontal submit-comment" action="add-article-comment.php" method="POST" role="form">
					...
					<div class="controls">
						<textarea dir="auto" class="col-xs-12 col-sm-10 col-md-10 col-lg-10" name="body" id="body" required
						          placeholder=" نظرتان را اینجا وارد کنید و این تگ ها هم قابل استفاده اند <strong><pre><em><p>"></textarea>
					</div>
