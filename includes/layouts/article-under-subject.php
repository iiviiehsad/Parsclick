...
<h2>
	...
	if (Article::count_recent_articles_for_subject($current_subject->id, FALSE) > 0) {
		...
		echo '<small><span class="label label-as-badge label-info">' .
			convert(Article::count_recent_articles_for_subject($current_subject->id, FALSE)) .
			' مقاله جدید</span></small>';
	}
	if (Article::count_invisible_articles_for_subject($current_subject->id) > 0) {
		...
		echo '<small><span class="label label-as-badge label-danger">' .
			convert(Article::count_invisible_articles_for_subject($current_subject->id)) .
			' مقاله مخفی</span></small>';
	}
	?>
</h2>
<ul>
