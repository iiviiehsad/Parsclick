...
global $json; ?>
	...
	<h5>
		...
		<?php echo isset($current_course->author_id) ?
			htmlentities(Author::find_by_id($current_course->author_id)->full_name()) : ''; ?>
	</h5>
	<h5>
...
<?php if ($session->is_admin_logged_in() || $session->is_author_logged_in()): ?>
	<h5>
		...
		<?php echo $current_course->visible ==
...
<?php if ( ! empty($current_course->content)): ?>
	<p>
		<?php echo nl2br(strip_tags($current_course->content, '<strong><em><p><code><pre><mark><kbd><ul><ol><li><img><a>')); ?>
	</p>
<?php endif; ?>