...
class ErrorTest extends TestCase {
	...
	{
		try {
			throw new Error\Api("hello", 500, "{'foo':'bar'}", array('foo' => 'bar'));
	...
	{
		try {
			throw new Error\Api("hello", 500, "{'foo':'bar'}", array('foo' => 'bar'), array('Request-Id' => 'req_bar'));
			$this->fail("Did not raise error");
		} catch(Error\Api $e) {
	...
	{
		try {
			throw new Error\Card("hello", "some_param", "some_code", 400, "{'foo':'bar'}", array('foo' => 'bar'));
			$this->fail("Did not raise error");
		} catch(Error\Card $e) {
