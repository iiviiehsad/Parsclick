		...
		if ($resp->isSuccess()) {
			if ($session->csrf_token_is_valid() && $session->csrf_token_is_recent()) {
				if ( ! has_presence($_POST['username']) || ! has_presence($_POST['password'])) {
					...
					$member->username   = strtolower(trim($_POST['username']));
					$member->password   = $member->password_encrypt($_POST['password']);
					$member->first_name = ucwords(strtolower(trim($_POST['first_name'])));
					$member->last_name  = ucwords(strtolower(trim($_POST['last_name'])));
					$member->gender     = trim($_POST['gender']);
					$member->address    = ucwords(strtolower(trim($_POST['address'])));
					$member->city       = ucwords(strtolower(trim($_POST['city'])));
